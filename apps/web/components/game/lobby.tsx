...
export function MatchLobby() {
    ...
    // SINGLE: auto-emit PLAYER_READY after 2s so the lobby is briefly visible
    useEffect(() => {
            ...
            >
                ...
                <div className="text-center space-y-3">
                    <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter">
                        {isSingle ? "Arena Ready" : sessionId ? "Opponent Found" : "Searching Arena"}
                    </h2>
                ...
                {isSingle && (
                    <motion.div className="flex flex-col items-center gap-3"
                        ...
                        {!isReady ? (
                            <>
                                <div className="text-6xl font-black font-mono text-primary">{countdown}</div>
                                <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Starting in…</p>
                            </>
                        ) : (
                            <div className="flex items-center gap-3 px-5 py-3 border-2 border-foreground bg-accent shadow-retro-sm">
                                ...
                                <span className="text-sm font-black uppercase tracking-widest text-accent-foreground">Launching Arena…</span>
                            </div>
                        )}
                    </motion.div>
                )}
                ...
                {!isSingle && !sessionId && (
                    ...
                    >
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">Matchmaker Queued</span>
                    </motion.div>
                )}
                ...
                {!isSingle && sessionId && (
                    <motion.button
                        className={`arcade-btn px-10 py-5 border-2 border-foreground shadow-retro text-lg font-black uppercase tracking-widest flex items-center gap-3 ${
                            isReady ? "bg-accent text-accent-foreground cursor-default" : "bg-primary cursor-pointer"
                        }`}
                    ...
                    >
                        {isReady ? <><CheckCircle2 className="size-5" />Waiting for Opponent…</> : "Ready Up"}
                    </motion.button>
                )}
            </motion.div>
        </div>
